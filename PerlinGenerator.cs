...
{     
    class PerlinGenerator
    {
        ...
        public static double Perlin(double x, double y)
        {
            ...
            double u = MathHelper.SmoothStep(xf);
            double v = MathHelper.SmoothStep(yf);

            int aa, ab, ba, bb;
            aa = lookupTable[lookupTable[xi] + yi];
            ab = lookupTable[lookupTable[xi] + inc(yi)];
            ba = lookupTable[lookupTable[inc(xi)] + yi];
            bb = lookupTable[lookupTable[inc(xi)] + inc(yi)];
        ...
        private static int[] InitialiseLookupTable(int cap, int randomSeed = -1)
        {
            ...
            {
                int random = rnd.Next(numbers.Count - 1);
                temp.Add(numbers[random]);
